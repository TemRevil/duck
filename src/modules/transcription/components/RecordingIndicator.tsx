...
export const RecordingIndicator: React.FC = () => {
    ...
    return (
        <div className="fixed bottom-6 left-6 right-6 z-50 md:left-auto md:right-6 md:max-w-sm">
            <GlassCard className={`p-4 flex items-center gap-3 ${currentStatus === 'error' ? 'bg-red-500/10 border-red-500/30' :
                    isProcessingActive ? 'bg-amber-500/10 border-amber-500/30' :
                        'bg-tropical-teal/10 border-tropical-teal/30'
                }`}>
                <div className={`flex-shrink-0 ${currentStatus === 'error' ? 'text-red-500' :
                ...
                <div className="flex-1">
                    <div className={`text-sm font-semibold ${currentStatus === 'error' ? 'text-red-600' :
